...
# Phase 2: If none found, fallback to known manufacturer aliases (ASUS, Elan, Synaptics)
if [ -z "$DEVICE" ]; then
    DEVICE=$(hyprctl devices -j | jq -r '.mice[] | select(.name | test("synaptics|asif|elan|asue"; "i")) | .name' | head -1)
fi
