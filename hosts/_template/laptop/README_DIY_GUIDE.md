...
Welcome to your new "Donor Laptop" setup! This template directory contains everything you need to customize your specific physical machine **without** breaking your Core dotfiles.
...
This is the heart of your machine. Use this to bind physical hardware keys (like `Fn` keys) to Core scripts.
*   **Example:** If your laptop has a dedicated "Cooler Boost" button, you bind it here.
*   **Example:** Launching Bluetooth or Battery applets specifically for laptops.

### 2. `monitor.conf` (Screen Layout)
Every screen is different. Do not leave this on `auto` if you want a reliable setup.
*   Run `hyprctl monitors all` in the terminal to find your monitor's name (e.g., `eDP-1`).
...
If this laptop is weak, turn off blur and animations in `user-visuals.conf`.
...
*   **Example:** To make this laptop dim after 1 minute (while keeping your desktop at 10 minutes), simply add `timeout = 60` to `hypridle-host.conf`.
