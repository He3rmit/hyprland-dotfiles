...
chmod +x "$DOTFILES_DIR/core/waybar/scripts/"* 2>/dev/null || true
chmod +x "$DOTFILES_DIR/hyprland/scripts/"* 2>/dev/null || true
chmod +x "$DOTFILES_DIR/core/swaync/scripts/"* 2>/dev/null || true
...
if [[ "$HAS_TOUCHPAD" == "true" ]]; then
    ...
    if [[ ! -f "$TOUCHPAD_CONF" ]]; then
        ...
        DETECTED_TP=$(hyprctl devices -j 2>/dev/null | jq -r '.mice[] | select(.name | test("touchpad"; "i")) | .name' | head -1)
        if [[ -z "$DETECTED_TP" ]]; then
            DETECTED_TP=$(hyprctl devices -j 2>/dev/null | jq -r '.mice[] | select(.name | test("synaptics|asif|elan|asue"; "i")) | .name' | head -1)
        fi
        [[ -z "$DETECTED_TP" ]] && DETECTED_TP="touchpad"
