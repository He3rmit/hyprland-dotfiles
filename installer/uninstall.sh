...
if [[ "$MODULES" == *"Unstow Configs"* ]] || [ "$FULL_EJECT" = true ]; then
    ...
    rm -f "$HOME/.config/hypr/modules/monitor.conf"
    rm -f "$HOME/.config/hypr/modules/nvidia.conf"
...
rm -f "$HOME/.config/hypr/touchpad.conf"
rm -f "$HOME/.config/hypr/modules/colors.conf"
